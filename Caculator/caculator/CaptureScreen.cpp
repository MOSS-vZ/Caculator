...
void CaptureScreen::init() {
    ...
    std::cout << "CaptureScreen init done (with real camera)" << std::endl;
}
...
void CaptureScreen::captureAndProcess() {
    if (m_frameMat.empty()) {
        std::cout << "No frame to capture" << std::endl;
        return;
    }

    // 保存拍照原图
    clearPhotoFolder();
    std::string filename = "photo/photo_" +
        std::to_string(std::chrono::system_clock::now().time_since_epoch().count()) +
        ".bmp";
    cv::imwrite(filename, m_frameMat);
    std::cout << "Photo saved: " << filename << std::endl;

    // ---- 真实批改 ----
    auto gr = gradeFrame(m_frameMat);
